...
class DataCalculator():
    ...
    def calculate_actual_energy(self) -> pd.DataFrame:
        ...
        # pd.Series.diff() computes the difference; dt.total_seconds() converts to seconds, then divided by 3600 to get hours
        time_diff_hours = self.df['created_at'].diff().dt.total_seconds().fillna(0) / 3600.0

        # Energy (kWh) = Power (kW) * Time (hours)
        # For the first row where diff is 0, we can estimate or set energy based on average time step or 0
        ...
        if len(self.df) > 1 and time_diff_hours.iloc[1] > 0:
            self.df.iloc[0, 'energy_actual_kw'] = self.df.loc[0, 'power_kw'] * time_diff_hours.iloc[1]
